...
var AppComponent = (function () {
    function AppComponent(store) {
        ...
        this.person$ = new Subject_1.Subject()
            .map(function (value) { return ({ payload: value, type: reducers_1.ADVANCE }); });
        this.recall$ = new Subject_1.Subject();
        this.time = store.select('clock');
        this.people = store.select('people');
        Observable_1.Observable.merge(this.click$, this.seconds$, this.person$, this.recall$
            .withLatestFrom(this.time, function (_, y) { return y; })
            .map(function (time) { return ({ type: reducers_1.RECALL, payload: time }); }))
